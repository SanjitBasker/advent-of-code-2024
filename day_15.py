...
def part1(
    ...
    for a in actions:
        ...
        neighbor = (
            bisect_right(to_search.items(), gen_max(y + sum(a.value))) - 1
        )
        ...
        if neighbor_item[1][0] == Space.FREE:
            ...
            if 0 <= (following := neighbor + sum(a.value)) < len(to_search):
                ...
                if following_item[1][0] == Space.FREE:
                    ...
                    replace(
                        to_replace[y + sum(a.value)], x, Space.FREE, Space.BOX
                    )
                    replace(
                        to_replace[box_dest_coord], x, Space.BOX, Space.FREE
                    )
                    pos = move(pos, a.value)
...
def double_rows_cols(
    ...
    new_rows = [
        SortedDict(
            (2 * start, (space, 2 * fin))
            for (start, (space, fin)) in row.items()
        )
        for row in rows
    ]
                ...
                if following_item[1][0] == Space.FREE:
                    ...
                    if a.value[1]:
                        ...
                        if obstacles:
                            ...
                            for i, j in obstacles:
                                replace(
                                    rows[i + sum(a.value)], j, Space.BOX, None
                                )
                                replace(
                                    rows[i + sum(a.value)],
                                    j + 1,
                                ...
                                replace(
                                    cols[j], i + sum(a.value), Space.BOX, None
                                )
                                replace(
                                    cols[j + 1],
                                    i + sum(a.value),
