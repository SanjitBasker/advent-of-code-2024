...
def parse(lines):
  ...
  start = next(
    itertools.chain.from_iterable(
      ((i, j) for j, c in enumerate(r) if c == "S") for i, r in enumerate(rows)
    )
  )
  end = next(
    itertools.chain.from_iterable(
      ((i, j) for j, c in enumerate(r) if c == "E") for i, r in enumerate(rows)
    )
  )
...
class VisitRecord:
  ...
  def serialize(self, n: int):
    return (
      ...
      + (-3 * self.arrival_direction[0] + -1 * self.arrival_direction[1] + 3)
      // 2
    )
...
def dijkstra4(traversable, start):
  ...
  while frontier:
    ...
    for ds, dx, dy in turns(vr.arrival_direction):
      ...
      if not traversable[visit.loc[0]][visit.loc[1]]:
        continue
      if not visited[visit.serialize(n)] or visited[visit.serialize(n)] > visit:
...
def part2(traversable, start, end):
  ...
  stack = [d for d in dests if d and d.distance == min(filter(None, dests)).distance]
  visited = set(stack)
    ...
    for ppos in predecessor_positions:
      for i in range(4):
        ...
        turn_penalty = (
          ...
          if sum(
            i * j
            for i, j in zip(vr.arrival_direction, prev_vr.arrival_direction)
          )
          == 0
          else 0
        )
